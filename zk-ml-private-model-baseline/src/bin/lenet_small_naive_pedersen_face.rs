...
fn main() {
    ...
    //actually these parameters are for u8 model. we use this simply for checking the number of constraints.
    let (conv1_w, conv2_w, conv3_w, fc1_w, fc2_w): (
        ...
        Vec<Vec<i8>>,
        Vec<Vec<i8>>,
    ) = read_lenet_small_inputs_i8_face();
    let x: Vec<Vec<Vec<Vec<i8>>>> = read_face_recognition_92_112_input_i8();
    //let person_feature_vector = read_vector1d_i8("pretrained_model/LeNet_ORL_pretrained/person_feature_vector.txt".to_string(), 40);

    println!("LeNet small baseline on face dataset");
    ...
    let flattened_z1d_u8 = flattened_z1d
        ...
        let conv1_weights_1d = conv1_weights_1d.iter().map(|x| (*x as i8) as u8).collect::<Vec<u8>>();
        ...
        let conv2_weights_1d = conv2_weights_1d.iter().map(|x| (*x as i8) as u8).collect::<Vec<u8>>();
        ...
        let conv3_weights_1d = conv3_weights_1d.iter().map(|x| (*x as i8) as u8).collect::<Vec<u8>>();
        ...
        let fc1_weights_1d = fc1_weights_1d.iter().map(|x| (*x as i8) as u8).collect::<Vec<u8>>();
        ...
        let fc2_weights_1d = fc2_weights_1d.iter().map(|x| (*x as i8) as u8).collect::<Vec<u8>>();
